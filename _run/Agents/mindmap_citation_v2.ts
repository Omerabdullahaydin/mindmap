...
// alanı güncellememişse eski değer kaybolmaz.
// ----------------------------------------------------------------------------
...
// ============================================================================
export async function mapNode(state: typeof MindmapStateAnnotation.State) {
  ...
  documents.forEach(doc => {
    ...
    // bunu paragrafları oluştururken ekliyor), buradan sayfa numarasını çıkar.
  ...
  const chunks: { pages: number[]; docs: Document[] }[] = [];
  ...
  for (let i = 0; i < sortedPages.length; i += PAGES_PER_CHUNK) {
    ...
    chunkPages.forEach(pageNum => {
      chunkDocs.push(...(pageMap.get(pageNum) || []));
    });
  ...
  const summaryPromises = chunks.map(async (chunk, chunkIdx) => {
    const chunkContent = chunk.docs
      .map((doc) => {
        ...
        return `[Sayfa ${pageNum}] ${doc.pageContent}`;
      })
      .join('\n\n');
...
// ============================================================================
export async function reduceNode(state: typeof MindmapStateAnnotation.State) {
  const topic = state.topic;
...
export async function toolNode(state: typeof MindmapStateAnnotation.State) {
  ...
  // Citation tool'unu al (bkz. Tools/rag_tool_citation.ts)
  const citationToolConfig = getCitationTool();

  try {
    // ---- ADIM 1: Kaynakları ekle ----
    console.log("🔍 Adding citations...");
    const citationResult = await citationToolConfig.func({
      mindmap_markdown: mindmap,
      items: items
    });
    // citationToolConfig.func bir JSON STRING döndürür (LangChain tool'ları
    // genelde metin döndürür), bu yüzden JSON.parse ile gerçek objeye çeviriyoruz.
    const citationParsed = JSON.parse(citationResult);
    const finalMindmap = citationParsed.mindmap_with_citations;
    const finalCitations = citationParsed.citations;
...
// ============================================================================
export async function expandItemsNode(state: typeof MindmapStateAnnotation.State) {
  ...
  // ------------------------------------------------------------------------
  // SAĞLAMLIK ADIMI (yeni eklendi): SÜRE NORMALİZASYONU
  // ------------------------------------------------------------------------
...
`;
    ...
    // için bir hata notu içeren yer tutucu metin dönüyor.
    try {
...
// ----------------------------------------------------------------------------
// Burada, yukarıda tanımladığımız 5 fonksiyonu birer "node" (düğüm) olarak
// bir "graph"a (yönlendirilmiş akış şemasına) ekliyoruz, sonra hangi düğümün
// hangi düğümden sonra çalışacağını (.addEdge) belirtiyoruz:
//
//   START -> docIngest -> map -> reduce -> tools -> expandItems -> END
//
// .compile() bu tanımı ÇALIŞTIRILABİLİR bir objeye dönüştürür
// (mindmapCitationAgentV2). Bunu .invoke({topic: "..."}) diye çağırdığınızda,
// yukarıdaki 5 adım OTOMATİK OLARAK sırayla çalışır.
// ============================================================================
const workflow = new StateGraph(MindmapStateAnnotation)
  .addNode("docIngest", docIngestNodeMindmap)
  .addNode("map", mapNode)
  .addNode("reduce", reduceNode)
  .addNode("tools", toolNode)
  .addNode("expandItems", expandItemsNode)
...
// (çalışma zamanında JS herhangi bir objeye serbestçe alan eklemeye izin verir).
(mindmapCitationAgentV2 as any).description = "PDF dökümanlarından Map-Reduce + LLM Tools ile citation ve HTML mindmap oluşturur";
